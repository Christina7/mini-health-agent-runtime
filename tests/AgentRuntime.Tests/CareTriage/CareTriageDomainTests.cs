...
public class CareTriageDomainTests
{
    ...
    private static SymptomKnowledgeBaseTool RealKbTool() => new(CareTriageDomain.DefaultKnowledgeBase());

    // Criteria #1 + #2: every cardiac chest phrase, on its own, scores 7 and triages UrgentCare.
    // Before the fix "chest pressure" was unknown to the KB, scored 0, and fell to SelfCare — the
    // unsafe direction — while "chest pain"/"chest tightness" correctly reached UrgentCare.
    [Theory]
    ...
    public async Task Cardiac_chest_phrase_alone_triages_urgent_care(string phrase)
    {
        ...
        ctx.AppendUser($"I have {phrase} right now");

        var result = await RealKbTool().ExecuteAsync(default, ctx, CancellationToken.None);

...
public class RoutingTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task Plan_app_path_serves_the_planner_console()
    {
        var client = _factory.CreateClient();

        var response = await client.GetAsync("/plan-app");

        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Equal("text/html", response.Content.Headers.ContentType?.MediaType);
        var html = await response.Content.ReadAsStringAsync();
        Assert.Contains("Health Planner", html); // the planner console's brand
    }
}
